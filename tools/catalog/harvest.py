...
KINDS = ("header", "symbol", "type_exists", "struct_member", "sizeof", "alignof")
...
# AC_CHECK_ALIGNOF: the type is only in the probe's own typedef.
_ALIGNOF = re.compile(r'ac_fn_c_compute_int\s+"\$LINENO"\s+"\(long int\) offsetof \(ac__type_alignof_, y\)"\s+' + _Q + r'\s+' + _Q, re.S)
_ALIGNOF_TYPE = re.compile(r'typedef struct \{ char x; (.+?) y; \} ac__type_alignof_;')
...
def harvest(text):
    ...
    for m in _ALIGNOF.finditer(text):
        inc = m.group(2)
        t = _ALIGNOF_TYPE.search(inc)
        if t and _literal(t.group(1)):
            typ = t.group(1)
            add(Entry("alignof", "ALIGNOF_" + as_tr_cpp(typ), typ, includes_of(inc)))
...
def catalog_macros(catalog_path):
    with open(catalog_path) as fh:
        return set(re.findall(r'"((?:HAVE_|SIZEOF_|ALIGNOF_)[A-Z0-9_]+)"', fh.read()))
...
def probe_source(entry, headers):
    ...
    if entry.kind == "alignof":
        return _inc(headers) + "int probe[_Alignof(%s) > 0 ? 1 : -1];\n" % entry.subject
...
def verify(entries, compiler, header_overrides=None):
    ...
    for e in entries:
        ...
        if e.kind in ("type_exists", "sizeof", "alignof"):
        ...
        if e.kind in ("sizeof", "type_exists", "alignof") and ok([]):
        ...
        if e.kind in ("sizeof", "alignof") and e.present:
            op = "sizeof" if e.kind == "sizeof" else "_Alignof"
...
SECTION_OF = {"header": "headers", "symbol": "symbols", "type_exists": "type_exists",
              "struct_member": "struct_members", "sizeof": "types", "alignof": "alignofs"}


def template_line(e):
    if e.macro.startswith("HAVE_DECL_"):
        return "#undef %s" % e.macro
    if e.kind in ("sizeof", "alignof"):
