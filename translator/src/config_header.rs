...
pub(crate) fn plan_config_header(
    ...
    for name in undef_names(template_text) {
        if let Some(label) = catalog_label(&name) {
            ...
            // intersects autoconf's m4 expansion with config.status's D[]
            // table. Neither source suffices alone.
            //
            // Ranked below the catalog, because a name the catalog carries
            // is a toolchain fact the CONSUMER must re-probe. Ranked above
            // make's variable database, which is a flat namespace seeded
            // with every macro configure knows and where a lookalike can
            // collide.
