...
fn read_codemodel_reply(
    ...
    // After the install()-declared pass: that one can add a header to
    // public_headers, which this one then treats as already accounted for
    // rather than re-adding to sources. Both run before rebasing, while
    // sources are still project-relative and include dirs absolute.
    inject_headers_found_on_include_path(&mut targets, &source_dir);
...
/// Quoted-form `#include "..."` targets named by a source file, in order.
///
/// Deliberately not a preprocessor: it does not evaluate `#if`, so it reports
/// includes inside a disabled branch too. That direction is the safe one here
/// — the caller uses this to decide which headers to STAGE, and staging a
/// header the compiler ends up not reading costs nothing, whereas missing one
/// breaks the build. json-c's `test1.c` is exactly that shape: the
/// `#include "parse_flags.h"` is unconditional while the only *call* into it
/// sits under `#ifdef TEST_FORMATTED`.
///
/// Only the quoted form, never `#include <...>`: the angle form names a
/// system/toolchain header, which the module must not carry.
fn quoted_includes(source_text: &str) -> Vec<&str> {
...
/// Attaches headers a target `#include`s but never listed, when the header
/// sits on one of that target's own include directories.
///
/// CMake needs no declaration for this: it compiles each `.c` and the
/// preprocessor finds the header on disk via `-I`, so the header is never a
/// build input and the File API never reports it. Bazel is the opposite — it
/// stages only DECLARED inputs into the sandbox, so the same header is absent
/// at compile time and the build fails with `file not found` even though the
/// `-I` flag is present and correct.
///
/// json-c is the live case: `tests/test1.c` is the only source CMake lists for
/// target `test1` (`add_executable(${TESTNAME} ${TESTNAME}.c)`), and
/// `parse_flags.h` is reachable purely through
...
/// project's `install()` declarations and marks headers PUBLIC on libraries.
/// This one has no such authority — the header is evidence of a private
/// compile-time dependency only — so it lands in `sources`, where a header
/// contributes nothing but its presence in the sandbox.
///
/// Scoped to the target's OWN include directories on purpose. Copying every
/// header under every include path would pull in unrelated files and make the
/// module a mirror of the source tree, which
/// `copy_referenced_sources`'s contract exists to prevent.
///
/// Paths are in the File API's frame (pre-rebase): `sources` are
/// project-relative, include dirs absolute — same as
/// `inject_unenumerated_installed_headers`.
fn inject_headers_found_on_include_path(targets: &mut [Target], source_dir: &Path) {
    for target in targets.iter_mut() {
        // Resolved up front: the same header is usually included by several of
        // a target's sources, and each miss would otherwise re-read the file.
        let mut discovered: Vec<String> = Vec::new();

        for relative in &target.sources {
            let absolute = if Path::new(relative).is_absolute() {
                normalize_lexically(Path::new(relative))
            } else {
                normalize_lexically(&source_dir.join(relative))
            };
            ...
            for included in quoted_includes(&text) {
                for dir in &target.includes {
                    let candidate = normalize_lexically(&Path::new(dir).join(included));
                    // Bound to the source tree: an include dir can point into
                    // the build directory, whose headers are reproduced by the
                    // config_header machinery rather than copied.
                    if !candidate.starts_with(source_dir) || !candidate.is_file() {
                        continue;
                    }
                    let Ok(project_relative) = candidate.strip_prefix(source_dir) else {
                        continue;
                    };
                    let project_relative = project_relative.to_string_lossy().into_owned();
                    if !target.sources.contains(&project_relative)
                        && !target.public_headers.contains(&project_relative)
                        && !discovered.contains(&project_relative)
                    {
                        discovered.push(project_relative);
                    }
                    break;
                }
            }
        ...
        // Sorted so the generated `srcs` doesn't depend on source-file or
        // include-dir iteration order — the reproducible-output invariant.
        discovered.sort();
...
";
  ...
  "tests": [
    ...
    #[test]
    fn quoted_includes_finds_the_quoted_form_only() {
        ...
        assert_eq!(
            quoted_includes(text),
            vec!["parse_flags.h", "spaced.h"],
            "the angle form names a toolchain header the module must not carry; \
             whitespace after # and after `include` is legal C"
        );
    }

    #[test]
    fn quoted_includes_reports_an_include_inside_a_disabled_branch() {
        // Deliberate: this is not a preprocessor. json-c's test1.c has an
        // unconditional #include whose only *call* sits under an #ifdef, and
        // staging a header the compiler skips is harmless while missing one
        // breaks the build.
        let text = "#ifdef NEVER_DEFINED\n#include \"gated.h\"\n#endif\n";
        assert_eq!(quoted_includes(text), vec!["gated.h"]);
    }

    // bzl-fxa.18: json-c's `test1` lists only test1.c, and parse_flags.h is
    // reachable purely through the target's own include dir. CMake finds it on
    // disk; Bazel stages only declared inputs, so it must become a source.
    #[test]
    fn a_header_included_from_an_include_dir_is_added_to_sources() {
        let dir =
            std::env::temp_dir().join(format!("bzlf_incscan_{}_{}", std::process::id(), line!()));
        let tests_dir = dir.join("tests");
        fs::create_dir_all(&tests_dir).unwrap();
        fs::write(
            tests_dir.join("test1.c"),
            b"#include \"parse_flags.h\"\n#include <stdio.h>\n",
        )
        .unwrap();
        fs::write(tests_dir.join("parse_flags.h"), b"#pragma once\n").unwrap();
        // Present on the include path but included by nobody.
        fs::write(tests_dir.join("unreferenced.h"), b"#pragma once\n").unwrap();
        let src = dir.to_string_lossy().into_owned();
        ...
        inject_headers_found_on_include_path(&mut targets, Path::new(&src));
        ...
        assert_eq!(
            ...
            "the included header must be declared so Bazel stages it into the sandbox; \
             `unreferenced.h` sits on the same include dir and must NOT be swept in"
        );
    }

    // The direction that must NOT inject: a header the target already lists,
    // and one that isn't on this target's include path at all.
    #[test]
    fn a_header_not_on_the_targets_include_path_is_not_added() {
        let dir =
            std::env::temp_dir().join(format!("bzlf_incscan_{}_{}", std::process::id(), line!()));
        let tests_dir = dir.join("tests");
        ...
        .unwrap();
        fs::write(tests_dir.join("listed.h"), b"#pragma once\n").unwrap();
        fs::write(other_dir.join("far_away.h"), b"#pragma once\n").unwrap();
        let src = dir.to_string_lossy().into_owned();
        ...
        let mut targets = vec![{
            ...
            t.kind = TargetKind::Executable;
            t.sources.push("tests/test1.c".to_string());
            // Already enumerated by CMake — must not be added twice.
            t.sources.push("tests/listed.h".to_string());
            t
        }];
        ...
        assert_eq!(
            ...
            vec![
                "tests/test1.c".to_string(),
                "tests/listed.h".to_string()
            ],
            "an already-listed header must not be duplicated, and far_away.h — real, \
             but not on THIS target's include dirs — must not be pulled in"
        );
    }
