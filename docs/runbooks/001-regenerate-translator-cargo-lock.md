...
`cargo`-generated `Cargo.lock` (see that extension's `crate.from_cargo`
block in the repo root's `MODULE.bazel`). `rules_rs` explicitly does
...
its source directly rather than trusting README summaries, per
