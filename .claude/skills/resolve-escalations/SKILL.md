...
## What the item and the recipes are for

Each `needs_attention/<NNN>-<slug>.md` describes one gap in **this** project.
Each `resolutions/<shape>.md` describes how that *shape* of gap is usually
closed. When they disagree, **the item wins** — it knows the project and the
recipe does not.

A recipe is a shape to adapt, never a patch to apply. It cannot know this
project's macro names, header layout, or release.

Read the item's machine-readable header (`kind`, `subject`) to find the
matching recipe; `ls resolutions/` is the index.
